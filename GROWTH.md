...
hypothesis
  ...
  -> browser-agent cohort packets
  -> real product events in PostHog
...
For `growth`, the Wizard lesson applies to instrumentation and cohort execution, not to the whole product. `growth` should be Stripe-like at the top level and Wizard-like for bounded workflows that need an agent to interpret or exercise an app.
...
That should not remain an ad hoc trick. It should become a first-class `growth` concept: an agent cohort run.
...
6. prepare bounded browser-agent cohort runs
...
growth schema cohort-report --json
```
...
growth env set --key POSTHOG_PROJECT_ID --value ... --json
growth connector auth check posthog --json
```

Values should be redacted by default. Presence, scopes, host, and project IDs can be surfaced safely.

### 5.5 Prepared Agent Runs Before Launched Agent Runs
...
```bash
growth cohort prepare demand-driven-onboarding --agents 4 --browser --json
```

Later:

```bash
growth cohort run <run_id> --backend claude-code --json
growth cohort run <run_id> --backend codex --json
growth cohort run <run_id> --backend manual --json
```
...
```json
{
  ...
  "agent_run_id": "cohort_123_agent_1"
}
```
...
```json
{
  ...
  "version": "0.1",
  "error": {
    "code": "POSTHOG_MISSING_QUERY_READ_SCOPE",
    "message": "PostHog API key is missing query:read scope.",
    "details": {
      "connector": "posthog",
      "required_scope": "query:read"
    }
  },
  "warnings": [],
  "next_steps": [
    "Update the PostHog personal API key to include query:read.",
...
```json
{
  ...
  "workflow": [
    ...
    "growth cohort prepare <id> --agents 4 --json",
...
```json
{
  ...
  "required_contract": {
    "assignment": {
      "stable_by": "user_id",
      "properties": ["experiment_id", "variant", "user_id"]
    },
    "events": [
      {
        "event": "signup_completed",
        "required_properties": ["experiment_id", "variant", "user_id", "session_id", "timestamp"]
      },
      {
        "event": "second_app_activated",
        "required_properties": ["experiment_id", "variant", "user_id", "first_app", "second_app"]
      }
    ],
    "agent_traffic": {
...
growth connector test posthog --sample sample.json --json
growth connector auth check posthog --json
growth connector auth set posthog --json
```
...
- explicit `experiment_id` and `variant` override assignment lookup
...
growth pull <experiment_id> --source posthog --run <run_id> --json
```

Requirements:

- require `after` unless pulling by run
- store pull record
- dedupe events
- dedupe assignments
- report drop reasons
...
growth analyze <experiment_id> --run <run_id> --json
```
...
```json
{
  ...
  "next_steps": [
    ...
    "Use cohort reports for qualitative UX issues."
  ]
}
```
...
```bash
growth cohort prepare <experiment_id> --agents 4 --browser --json
growth cohort show <run_id> --json
growth cohort attach-report <run_id> --agent 1 --file agent-1.report.json --json
growth cohort complete <run_id> --json
growth cohort pull <run_id> --source posthog --json
growth cohort audit <run_id> --json
```
...
- URLs with:
  ...
  - optional forced variant only if requested
- prompt files
- tool policy files
- report schemas
- launch instructions
...
```text
.growth/runs/cohort_20260430T150000Z/
  run.json
  batch-start.txt
  agent-packets/
    agent-1.prompt.txt
...
```bash
growth audit <run_id> --json
growth audit <run_id> --markdown
```
...
- synthetic traffic from real user traffic
...
interface GrowthRun {
  id: string;
  type: "agent_cohort" | "instrumentation" | "pull" | "analysis";
  experiment_id?: string;
...
```json
{
  ...
  "connectors": {
    "posthog": {
      ...
      "required_env": ["POSTHOG_PERSONAL_API_KEY", "POSTHOG_PROJECT_ID"],
...
{"ts":"2026-04-30T00:05:00Z","command":"growth cohort prepare demand-driven-onboarding --agents 4","ok":true,"actor":"agent","run_id":"cohort_..."}
```
...
- Use `growth cohort prepare`, not ad hoc browser-agent prompts.
...
Use the `growth` CLI as the control plane for experiments, connectors, synthetic cohorts, pulls, analysis, and audit.
...
- browser-agent cohort target is usually local dev server URL
...
- required scopes:
  - `query:read`
  - `project:read`
  - optional `insight:read`
  - optional `dashboard:write` if creating assets

It should fail before a long run if scopes are missing.
...
### 16.6 Complete Cohort And Pull Events

```bash
growth cohort complete cohort_20260430T150000Z --json
growth cohort pull cohort_20260430T150000Z --source posthog --json
```
...
growth audit cohort_20260430T150000Z --markdown
```
...
6. Add cohort pull by run window.
7. Add cohort audit report.
...
6. Should cohort runs support forced variants, or should forced variants be considered a special test mode?
...
5. `growth` should treat browser-agent cohorts as first-class but synthetic.
